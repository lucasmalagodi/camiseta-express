...
exports.productService = {
    ...
    async update(id, data) {
        ...
        const verifyResult = await (0, db_1.query)('SELECT active FROM products WHERE id = ?', [id]);
    ...
    async findById(id) {
        const results = await (0, db_1.query)('SELECT id, category_id as categoryId, name, description, quantity, active, created_at as createdAt, updated_at as updatedAt FROM products WHERE id = ?', [id]);
    ...
    async findAll(filters) {
        ...
        const countResults = await (0, db_1.query)(countSql, values);
    ...
    async findAllWithDetails(filters, agencyId) {
        let whereClause = 'WHERE 1=1';
        const values = [];
        if (filters) {
            if (filters.categoryId !== undefined) {
        ...
        // Buscar produtos
        const sql = `SELECT id, category_id as categoryId, name, description, quantity, active, created_at as createdAt, updated_at as updatedAt FROM products ${whereClause} ORDER BY created_at DESC`;
        const products = await (0, db_1.query)(sql, values);
        ...
        if (data.length > 0) {
            ...
            let purchasesByLot = {}; // key: "productId-priceId"
            if (agencyId && productIds.length > 0) {
                ...
                `;
                const purchasesByLotRows = await (0, db_1.query)(purchasesByLotSql, [...productIds, agencyId]);
                purchasesByLotRows.forEach((row) => {
            ...
            const productsWithDetails = data.map(product => {
                ...
                if (sortedPrices.length > 0) {
                    for (let i = 0; i < sortedPrices.length; i++) {
                        ...
                        if (unitsInThisLot >= quantidadeCompra)
                            continue;
                        loteDisponivel = lote;
                        podeComprar = true;
                        break;
                ...
                const maiorPreco = prices.reduce((max, p) => Number(p.value) > Number(max.value) ? p : max, prices[0] || { value: 0 });
                return {
                    ...product,
                    images,
                    prices,
    ...
    async findByIdWithDetails(id, agencyId) {
        ...
        const allImages = await (0, db_1.query)(imagesSql, [id]);
        // Buscar todos os preços
