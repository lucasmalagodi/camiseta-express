...
function normalizeDate(value) {
    ...
    if (typeof value === 'string') {
        ...
        const tryParseDate = (part1, part2, part3) => {
...
async function processSpreadsheet(filePath) {
    ...
    try {
        ...
        if (jsonData.length > 3) {
            ...
            const row4Headers = row4.map((h) => String(h || '').trim().toLowerCase());
        ...
        const requiredFields = ['cnpj', 'points'];
        ...
        console.log(`📊 Resumo de colunas mapeadas:`, Object.entries(columnIndices).map(([field, index]) => `${field}=[${index}]"${headers[index] || 'N/A'}"`).join(', '));
        ...
        const requiredDbFields = ['cnpj', 'points'];
        ...
        if (missingFields.length > 0) {
            ...
            const foundColumns = Object.entries(COLUMN_MAPPING)
                ...
                .map(([spreadsheetCol]) => spreadsheetCol);
            throw new Error(`Colunas obrigatórias não encontradas na planilha: ${missingColumns.join(', ')}. ` +
