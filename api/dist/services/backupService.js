...
exports.backupService = {
    ...
    async createBackupRecord(type) {
        ...
        const result = await (0, db_1.query)(`INSERT INTO backups (file_name, file_path, type, status, created_at) 
             VALUES (?, ?, ?, 'PROCESSING', NOW())`, [fileName, filePath, type]);
        return result.insertId;
    },
    /**
     * Atualiza o status do backup
     */
    async updateBackupStatus(id, status, fileSize, errorMessage) {
        ...
        await (0, db_1.query)(`UPDATE backups SET ${updates.join(', ')} WHERE id = ?`, [...values, id]);
    },
    /**
     * Gera o dump do MySQL usando conexão direta (mais confiável que docker exec)
     */
    async generateDatabaseDump(dumpPath) {
        ...
        if (dbHost === 'mysql') {
            ...
            try {
                ...
                fs_1.default.writeFileSync(dumpPath, stdout);
                return;
            }
            catch (dockerError) {
     ...
     */
    async generateDumpViaConnection(dumpPath, dbName) {
        ...
        try {
            ...
            const [tables] = await connection.execute(`SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA = ? ORDER BY TABLE_NAME`, [dbName]);
                    ...
                    if (rows.length > 0) {
                        ...
                        for (let i = 0; i < rows.length; i += batchSize) {
                            ...
                            const values = batch.map((row) => {
                                const rowValues = Object.keys(rows[0]).map((col) => {
                                    const val = row[col];
            ...
            // Escrever arquivo
            fs_1.default.writeFileSync(dumpPath, dumpLines.join('\n'), 'utf8');
        }
        finally {
            connection.release();
        }
    ...
    async createBackup(type) {
        ...
        try {
            ...
            const dumpPath = path_1.default.join(tempDir, 'database.sql');
            ...
            const uploadsPath = path_1.default.join(assetsPath, 'product');
            if (fs_1.default.existsSync(uploadsPath)) {
                console.log(`[Backup ${backupId}] Copiando pasta de uploads...`);
                const uploadsBackupPath = path_1.default.join(tempDir, 'uploads');
                await execAsync(`cp -r "${uploadsPath}" "${uploadsBackupPath}"`);
            }
            // 3. Copiar outros assets importantes (se existirem)
            const spreadsheetsPath = path_1.default.join(assetsPath, 'spreadsheets');
            if (fs_1.default.existsSync(spreadsheetsPath)) {
                console.log(`[Backup ${backupId}] Copiando planilhas...`);
                const spreadsheetsBackupPath = path_1.default.join(tempDir, 'spreadsheets');
                await execAsync(`cp -r "${spreadsheetsPath}" "${spreadsheetsBackupPath}"`);
            }
            // 4. Criar arquivo de informações do backup
            const infoPath = path_1.default.join(tempDir, 'backup-info.txt');
            const infoContent = `Backup gerado em: ${new Date().toISOString()}
Tipo: ${type}
ID: ${backupId}
`;
            fs_1.default.writeFileSync(infoPath, infoContent);
            ...
            await this.updateBackupStatus(backupId, 'FAILED', undefined, error.message || 'Erro desconhecido');
     ...
     */
    async listBackups() {
        const results = await (0, db_1.query)(`SELECT * FROM backups ORDER BY created_at DESC`);
    ...
    async getBackupById(id) {
        const results = await (0, db_1.query)(`SELECT * FROM backups WHERE id = ?`, [id]);
        if (results.length === 0) {
            return null;
        }
        const row = results[0];
    ...
    async deleteBackup(id) {
        ...
        if (fs_1.default.existsSync(backup.file_path)) {
            fs_1.default.unlinkSync(backup.file_path);
        }
        // Deletar registro do banco
        await (0, db_1.query)('DELETE FROM backups WHERE id = ?', [id]);
    },
    /**
     * Aplica política de retenção (mantém apenas os últimos N backups automáticos)
     */
    async applyRetentionPolicy(keepCount = 7) {
        // Buscar backups automáticos ordenados por data (mais recentes primeiro)
        const results = await (0, db_1.query)(`SELECT id FROM backups 
    ...
    async validateBackup(id) {
        ...
        try {
            ...
            const stats = fs_1.default.statSync(backup.file_path);
            if (stats.size > 0) {
            ...
            try {
                ...
                const extractedFiles = fs_1.default.readdirSync(tempDir);
                ...
                // 4. Validar dump SQL
                const sqlDumpPath = path_1.default.join(tempDir, 'database.sql');
                if (fs_1.default.existsSync(sqlDumpPath)) {
                    const sqlContent = fs_1.default.readFileSync(sqlDumpPath, 'utf8');
                ...
                const uploadsPath = path_1.default.join(tempDir, 'uploads');
                if (fs_1.default.existsSync(uploadsPath)) {
                    checks.uploadsPresent = true;
                    const uploadsStats = fs_1.default.statSync(uploadsPath);
                    if (uploadsStats.isDirectory()) {
                        const uploadFiles = fs_1.default.readdirSync(uploadsPath);
