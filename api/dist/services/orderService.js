...
exports.orderService = {
    async create(agencyId, data) {
        ...
        try {
            ...
            for (const item of data.items) {
                ...
                if (item.variantId) {
                    const [variantResults] = await connection.execute('SELECT id, product_id as productId, model, size, stock, active FROM product_variants WHERE id = ? AND product_id = ? FOR UPDATE', [item.variantId, item.productId]);
            ...
            for (const item of orderItemsData) {
                await connection.execute('INSERT INTO order_items (order_id, product_id, product_price_id, product_variant_id, quantity, points_per_unit) VALUES (?, ?, ?, ?, ?, ?)', [orderId, item.productId, item.productPriceId, item.variantId, item.quantity, item.pointsPerUnit]);
                // Decrementar estoque: se tem variantId, decrementar da variação, senão do produto geral
                if (item.variantId) {
                    await connection.execute('UPDATE product_variants SET stock = stock - ?, updated_at = NOW() WHERE id = ? AND stock >= ?', [item.quantity, item.variantId, item.quantity]);
                }
                else {
            ...
            try {
                ...
                if (agency) {
                    ...
                    await notificationService.sendOrderNotification(orderId, agencyId, agency.name, totalPoints, orderUrl);
                }
            }
            catch (emailError) {
    ...
    async findMyOrders(agencyId) {
        const results = await (0, db_1.query)(`SELECT 
             ...
             WHERE o.agency_id = ?
             GROUP BY o.id
             ORDER BY o.created_at DESC`, [agencyId]);
        return Array.isArray(results) ? results.map((r) => ({
            id: Number(r.id),
    ...
    async findMyOrderById(orderId, agencyId) {
        ...
        const itemsResults = await (0, db_1.query)(`SELECT 
                oi.id,
                oi.order_id as orderId,
                oi.product_id as productId,
                oi.product_price_id as productPriceId,
             ...
             WHERE oi.order_id = ?
             ORDER BY oi.id ASC`, [orderId]);
        const items = Array.isArray(itemsResults) ? itemsResults.map((r) => ({
            id: Number(r.id),
            orderId: Number(r.orderId),
            productId: Number(r.productId),
            productPriceId: r.productPriceId ? Number(r.productPriceId) : null,
