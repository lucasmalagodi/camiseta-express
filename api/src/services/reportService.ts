...
const ALLOWED_FIELDS: Record<ReportSourceTable, string[]> = {
...
const NUMERIC_FIELDS: Record<ReportSourceTable, string[]> = {
...
const TABLE_RELATIONSHIPS: Record<string, { table: string; alias: string; on: string; type?: 'LEFT' | 'INNER'; requires?: string }> = {
...
const RELATED_TABLE_FIELDS: Record<string, string[]> = {
    'agencies': [
        'id', 'cnpj', 'name', 'email', 'phone', 'address', 'active',
        'branch', 'executive_name', 'created_at', 'updated_at'
    ],
 ...
 */
function isFieldAllowed(table: ReportSourceTable, field: string): boolean {
 ...
 */
function isNumericField(table: ReportSourceTable, field: string): boolean {
 ...
 */
function isRelatedFieldAllowed(relatedTable: string, field: string): boolean {
...
function getRequiredJoins(
    ...
    const addJoinWithDependencies = (joinKey: string) => {
 ...
 */
function getRelatedTableAlias(table: ReportSourceTable, relationshipKey: string): string | null {
 ...
 */
function buildQuery(table: ReportSourceTable, config: ReportConfig): { sql: string; params: any[] } {
    ...
    const getColumnPosition = (field: string): number => {
        let position = 1;
 ...
 */
export async function executeReport(reportId: number): Promise<any[]> {
 ...
 */
export async function createReport(dto: CreateReportDto, userId: number): Promise<Report> {
    try {
        ...
        for (const filter of config.filters) {
            ...
            if (parts.length === 2) {
                ...
                if (!relatedTableAlias) {
                    ...
                    throw new Error(
                        `Relacionamento não encontrado no filtro: ${relationshipKey}. ` +
                ...
                if (!isRelatedFieldAllowed(relatedTableAlias, field)) {
                    ...
                    throw new Error(
                        `Campo não permitido no filtro: ${relatedTableAlias}.${field}. ` +
 ...
 */
function parseConfigJson(configJson: any): any {
...
export async function deleteReport(id: number): Promise<void> {
    ...
    const widgetsDeleted = await query(
        `DELETE FROM dashboard_widgets WHERE report_id = ?`,
        [id]
    ) as any;
    console.log(`[deleteReport] Widgets deletados: ${widgetsDeleted.affectedRows || 0}`);

    // Deletar o relatório
    const result = await query(`DELETE FROM reports WHERE id = ?`, [id]) as any;
 ...
 */
export function getAvailableFields(table: ReportSourceTable): {
    dimensions: string[];
    metrics: string[];
    relatedTables: Array<{ key: string; table: string; alias: string; fields: string[] }>;
} {
    const relatedTables: Array<{ key: string; table: string; alias: string; fields: string[] }> = [];
